-- First, let's see what the current constraint allows and fix it
-- The error suggests the lobby_type values don't match the constraint
...
-- Add constraint with the correct lobby type values that match our code
ALTER TABLE public.lobbies ADD CONSTRAINT lobbies_lobby_type_check 
CHECK (lobby_type IN ('quiz-2', 'quiz-4'));

-- Also add a profiles table with 6-digit user IDs for invitation system
...
-- Add a 6-digit user_code to profiles table
...
-- Create trigger to auto-generate user codes for existing and new profiles
...
-- Update existing profiles without user codes
