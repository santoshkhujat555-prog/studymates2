...
const Profile = () => {
  ...
  // Fetch the profile (own or others)
  const fetchProfile = async () => {
    if (!user) return;

    const idToFetch = isOwnProfile ? user.id : userId;
  ...
  const fetchFriendRequestStatus = async () => {
    ...
    if (data) setRequestStatus(data.status as 'pending' | 'accepted');
  };
  ...
  const sendFriendRequest = async () => {
    ...
    if (error) {
      ...
      toast({ title: 'Request Sent', description: 'Friend request has been sent' });
      ...
      <main className="container mx-auto px-4 py-8 max-w-2xl">
        <Card>
          ...
          <CardContent className="space-y-6">
            {/* User Code Section */}
            {isOwnProfile && profile?.user_code && (
            ...
            {!isOwnProfile && profile?.user_id && (
              <div className="text-center mb-4">
                ...
                {requestStatus === 'pending' && <Button disabled>Request Pending</Button>}
                {requestStatus === 'accepted' && <Button disabled>Friends</Button>}
              </div>
            )}
